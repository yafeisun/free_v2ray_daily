...
def main():
    """主函数"""
    logger = get_logger("proxyqueen")
    file_handler = FileHandler()
    
    try:
        # 初始化收集器
        site_name = 'proxyqueen'
        collector = ProxyQueenCollector(WEBSITES[site_name])
        ...
        if not article_url:
            logger.info(f"{site_name}: 今日未更新文章")
            # 保存信息到文件
            ...
            logger.info(f"文章信息已保存到 {article_file}")
            return True
