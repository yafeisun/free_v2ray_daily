    ...
    def test_all_nodes(self, nodes, min_success_sites=None):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(future_to_node)):
                ...
                try:
                    # 设置超时保护
                    is_valid, success_count, success_sites, tcp_only = future.result(timeout=TIMEOUT * 5 + 10)

                    if is_valid:
                        valid_nodes.append(node)
                        self.logger.info(f"✓ 节点有效 ({len(valid_nodes)}/{i+1}): TCP连通")
                    else:
                        self.logger.info(f"✗ 节点无效 ({i+1}/{len(nodes)}): TCP连接失败")
