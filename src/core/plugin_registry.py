...
class PluginRegistry:
    ...
    def discover_collectors(self, collectors_dir: str = "src.collectors") -> None:
        ...
        for filename in os.listdir(collectors_path):
            if (filename.endswith('.py') and 
                ...
                filename != 'base_collector.py' and
        ...
        self.logger.info(f"发现并注册了 {len(self._collectors)} 个收集器: {list(self._collectors.keys())}")
    ...
    def create_collector_instance(self, site_key: str, site_config: Dict) -> BaseCollector:
        """创建收集器实例"""
